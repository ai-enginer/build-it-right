...
const tones = ["Standard", "Formal", "Casual", "Creative", "Academic", "Simple"] as const;
...
const AIParaphraser = () => {
  ...
  const handleParaphrase = async () => {
    ...
    // Example: const res = await supabase.functions.invoke('ai-paraphrase', { body: { text: input, tone } });
    try {
      await new Promise(r => setTimeout(r, 1500));
      setOutput(`[API NOT CONNECTED] Placeholder paraphrase.\n\nConnect an AI API and update handleParaphrase in AIParaphraser.tsx.\n\nTone: "${tone}", Input: ${input.split(/\s+/).length} words.`);
    } catch {
      toast({ title: "Error", description: "Failed to paraphrase.", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };

  const relatedTools = tools.filter(t => t.categorySlug === "ai" && t.slug !== "ai-paraphraser").slice(0, 4);
    ...
    <SiteLayout>
      <div className="container py-8">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold text-foreground md:text-3xl">AI Paraphraser</h1>
          <p className="mt-1 text-sm text-muted-foreground">Rewrite text in different styles and tones. Maintain meaning while changing the wording.</p>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="relative">
            ...
            <textarea value={input} onChange={e => setInput(e.target.value)} placeholder="Enter text to paraphrase..."
          ...
          <div className="relative">
            <label className="mb-1.5 block text-xs font-medium text-muted-foreground">Paraphrased Text</label>
            <textarea value={output} readOnly placeholder="Paraphrased text will appear here..."
        ...
        <Button className="mt-4 w-full sm:w-auto" onClick={handleParaphrase} disabled={!input.trim() || loading}>
